...
Section \ref{sec:meth} presents the methodology of the paper. Section 4 discusses the results and Section 5 concludes
the study.
...
\subsection{Data}
Monthly FTSE 250 Index price data between Oct 1992 to March 2024 has been utilised for this study giving us a dataset with 379 observations.
...
announce a rate on a regular basis. To address the resulting irregularities the data was adjusted using \ref{alg:interest_rate_adjustment}.
...
extensively in this aspect (\ref{subsubsec: fore}). The advantages of radial basis function neural networks over other 
...
First, the dataset was normalised using a z-score normalisation, because neural networks work best with data
that is between a specified range (\citeboth{mendelsohn1993}).

The inputs into the network are 4-dimensional input vectors, \(\boldsymbol{\mathrm{x_i}}\),
...
The output is a one-dimensional output vector, \(\boldsymbol{\mathrm{y_i}}\), representing differenced monthly values in the FTSE250.
...
Following this 
it was necessary to program the radial basis function neural network into Python. The Bayesian kernel regression approach outlined by 
...
Based on this, the train-test process is then conducted:
\begin{enumerate}
    \item First, the optimal hyperparameters in the covariance function \eqref{eq: gimq} 
    ...
    \end{enumerate}
    \item Then, training inputs $\boldsymbol{\mathrm{x}}_1, \boldsymbol{\mathrm{x}}_2,\ldots \boldsymbol{\mathrm{x}}_{n}$
    ...
    \item The testing process is conducted similarly. The 
